...
const before = curryN(2, (trackFn, fn) => function (...args) {
  ...
  isFunction$1(trackFn) && evalWithNoCatch(trackFn, args);
 ...
 *     }
 * }*/
const track = partical => (target, key, descriptor) => {
...
const composeWith = curry((convergeFn, ops) => {
  if (isFunction$1 (ops)) {
    ops = [ops];
  }
...
const evolve = curry(evols => fn => function (...args) {
  const self = this;
  const memoizeFn = memoize$1(fn);
...
const identity = curry(fn => function (...args) {
  return fn.apply(this, args)
});
